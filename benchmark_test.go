...
func BenchmarkOneRoute(b *testing.B) {
	router := fwncs.New()
	router.GET("/ping", func(c fwncs.Context) {})
	runRequest(b, router, "GET", "/ping")
}

func BenchmarkRecoveryMiddleware(b *testing.B) {
	router := fwncs.New()
	router.Use(fwncs.Recovery())
...
func BenchmarkLoggerMiddleware(b *testing.B) {
	router := fwncs.New()
	router.Use(fwncs.Recovery(), fwncs.Logger())
	router.GET("/", func(c fwncs.Context) {})
	runRequest(b, router, "GET", "/")
}
...
func Benchmark5Params(B *testing.B) {
	router := fwncs.New()
	router.Use(func(c fwncs.Context) {})
	router.GET("/param/:param1/:params2/:param3/:param4/:param5", func(c fwncs.Context) {})
...
func BenchmarkOneRouteJSON(B *testing.B) {
	...
	router.GET("/json", func(c fwncs.Context) {
...
func runRequest(B *testing.B, r *fwncs.Router, method, path string) {
	// create fake request
...
func loadNCSRoutes(router *fwncs.Router, routes []*Route) {
	for _, r := range routes {
		switch r.Method {
		case "GET":
			router.GET(r.Path, handler(r.Method, r.Path))
		case "POST":
			router.POST(r.Path, handler(r.Method, r.Path))
		case "PATCH":
			router.PATCH(r.Path, handler(r.Method, r.Path))
		case "PUT":
			router.PUT(r.Path, handler(r.Method, r.Path))
		case "DELETE":
			router.DELETE(r.Path, handler(r.Method, r.Path))
		}
	}
}

func handler(method, path string) fwncs.HandlerFunc {
	return func(c fwncs.Context) {
		c.String(http.StatusOK, "OK")
	}
}

func BenchmarkNCSStatic(b *testing.B) {
	router := fwncs.New()
	loadNCSRoutes(router, static)
	benchmarkRoutes(b, router, static)
}

func BenchmarkNCSGitHubAPI(b *testing.B) {
	router := fwncs.New()
	loadNCSRoutes(router, githubAPI)
	benchmarkRoutes(b, router, githubAPI)
}

func BenchmarkNCSGplusAPI(b *testing.B) {
	router := fwncs.New()
	loadNCSRoutes(router, gplusAPI)
	benchmarkRoutes(b, router, gplusAPI)
}

func BenchmarkNCSParseAPI(b *testing.B) {
	router := fwncs.New()
	loadNCSRoutes(router, parseAPI)
